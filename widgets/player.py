...
class PlayerWindow(QtWidgets.QWidget):
    ...
    def __init__(self, ui_file, conf_file, *args, **kwargs):
        ...
        setting_conf_file = setting_ui_file.replace('/ui/setting.ui', '/config/setting.conf')
        ...
        self.pb_once.setStyleSheet('QPushButton {background-color: rgb(53, 53, 255);}')
        self.pb_loop.setStyleSheet('QPushButton {background-color: rgb(43, 43, 43);}')
    ...
    def pb_loop_cb(self):
        if self.loop:
            ...
            self.loop = True
            self.pb_loop.setStyleSheet('QPushButton {background-color: rgb(53, 53, 255);}')
            self.pb_once.setStyleSheet('QPushButton {background-color: rgb(43, 43, 43);}')
    ...
    def bag_info(self):
        ...
        if resp.stdout == '':
            ...
            for line in lines:
                ...
                if 'Duration:' in line:
                    self.duration = int(re.split('[:.]', line)[1])
    ...
    def pb_play_cb(self):
        ...
        self.start = self.sb_offset.value()
        ...
        self.player.setRosbag(rosbag_dir, self.duration)
        self.player.setSource(self.ros_path)
        self.player.setRate(rate)
        self.player.setStartOffset(self.start)
